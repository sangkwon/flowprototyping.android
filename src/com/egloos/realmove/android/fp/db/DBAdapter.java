...
public class DBAdapter {
	...
	private static final String CREATE_TBL_PROJECT = "CREATE TABLE " + TBL_PROJECTS + " (" + ID + " INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,"
			+ SUBJECT + " TEXT," + MAIN_IMAGE + " TEXT," + CREATED + " INTEGER," + UPDATED + " INTEGER);";
	...
	private static final String CREATE_TBL_LINK = "CREATE TABLE " + TBL_LINKS + " (" + ID + " INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT," + PAGE_ID
			+ " INTEGER," + EVENT + " TEXT," + ANIM + " TEXT," + TARGET_PAGE_ID + " INTEGER," + POS_LEFT + " INTEGER," + POS_TOP + " INTEGER,"
			+ POS_RIGHT + " INTEGER," + POS_BOTTOM + " INTEGER);";
	...
	public Project selectProject(int projectId) throws Exception {
		...
		try {
			...
			cursor = mDb.query(TBL_PROJECTS, projection, ID + "=" + projectId, null, null, null, null, null);
			if (cursor.moveToNext()) {
				Project project = new Project();
				project.setId(cursor.getInt(0));
				project.setSubject(cursor.getString(1));
	...
	public ArrayList<Link> selectLinks(int... pageIds) throws Exception {
		...
		try {
			String[] projection = new String[] {
					ID, PAGE_ID, EVENT, ANIM, TARGET_PAGE_ID, POS_LEFT, POS_TOP, POS_RIGHT, POS_BOTTOM
			};
