...
public class LinkImageEditView extends LinkImageView implements OnMenuItemClickListener {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				if (selected != null) {
					switch (getState()) {
						case SELECTING_LINK:
							if ((dx > -DRAG_THRESHOLD_X && dx < DRAG_THRESHOLD_X) || (dy > -DRAG_THRESHOLD_Y && dy < DRAG_THRESHOLD_Y)) {
								// ignore this
								break;
							}
							stopLongtapTimer();
	...
	protected void drawLink(Canvas canvas, Paint paint, Link link) {
		...
		if (selected != null && selected == link) {
			...
			float[] pts = new float[] {
					dstRect.getLeft(), dstRect.getTop(), dstRect.getRight(), dstRect.getTop(), dstRect.getRight(), dstRect.getTop(),
					dstRect.getRight(), dstRect.getBottom(), dstRect.getRight(), dstRect.getBottom(), dstRect.getLeft(), dstRect.getBottom(),
					dstRect.getLeft(), dstRect.getBottom(), dstRect.getLeft(), dstRect.getTop()
			};
